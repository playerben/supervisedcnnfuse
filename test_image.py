...
def _generate_fusion_image(model, strategy_type, img1, img2, mask1, mask2, img_b1, img_b2, img_d1, img_d2):
	# encoder
	# test = torch.unsqueeze(img_ir[:, i, :, :], 1)
	en_r = model.encoder(img1, mask1, img_b1, img_d1)
	# vision_features(en_r, 'ir')
	en_v = model.encoder(img2, mask2, img_b2, img_d2)
...
def run_demo(model, infrared_path, visible_path, output_path_root, index, fusion_type, network_type, strategy_type, ssim_weight_str, mode):
	# if mode == 'L':
	[ir_img, ir_mask, ir_b, ir_d] = utils.get_test_images(infrared_path, height=None, width=None, mode=mode)
	[vis_img, vis_mask, vis_b, vis_d] = utils.get_test_images(visible_path, height=None, width=None, mode=mode)
	# else:
	...
	ir_mask = Variable(ir_mask, requires_grad=False)
	vis_mask = Variable(vis_mask, requires_grad=False)
	ir_b = Variable(ir_b, requires_grad=False)
	vis_b = Variable(vis_b, requires_grad=False)
	ir_d = Variable(ir_d, requires_grad=False)
	vis_d = Variable(vis_d, requires_grad=False)
	dimension = ir_img.size()

	img_fusion = _generate_fusion_image(model, strategy_type, ir_img, vis_img, ir_mask, vis_mask, ir_b, vis_b, ir_d, vis_d)
	############################ multi outputs ##############################################
