...
def train(i, original_imgs_path):
	...
	for e in tbar:
		...
		for batch in range(batches):
			...
			[img, img_mask, img2, img_b, img_d] = utils.get_train_images_auto(image_paths, height=args.HEIGHT, width=args.WIDTH, mode=img_model)
			...
			img_d = Variable(img_d, requires_grad=False)
			...
			en = densefuse_model.encoder(img, img_mask, img_b, img_d)
			# decoder
			outputs = densefuse_model.decoder(en)
			# resolution loss
			# x = Variable(img.data.clone(), requires_grad=False)
			y = Variable(img2.data.clone(), requires_grad=False)
			...
			pixel_loss_value = 0.
			for output in outputs:
				ssim_loss_temp = ssim_loss(output, y, normalize=True)
				pixel_loss_temp = mse_loss(output, y)
				ssim_loss_value += (1-ssim_loss_temp)
				pixel_loss_value += pixel_loss_temp
			ssim_loss_value /= len(outputs)
			pixel_loss_value /= len(outputs)
